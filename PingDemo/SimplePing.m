 ...
 *  \param requiresChecksum Determines whether a checksum is caulated (IPv4) or not (IPv6).
...
/*! Caulates the offset of the ICMP header within an IPv4 packet.
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        ...
        icmpPtr->checksum  = 0;
        caulatedChecksum = in_cksum(icmpPtr, packet.length - icmpHeaderOffset);
        icmpPtr->checksum  = receivedChecksum;
        
        if (receivedChecksum == caulatedChecksum) {
